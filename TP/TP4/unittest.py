...
# Barcode inputs include checksum (last digit included)
def unit_test(barcode_inputs_with_check_sum, file_name):
    ...
    if len(barcode_inputs_with_check_sum) != 13 or not barcode_inputs_with_check_sum.isdigit():
        ...
        # Aspect 3: Check if checksum in barcode = checksum that is calculated via the function
...
# Here, just insert 13 digits of product barcode (include checksum)
